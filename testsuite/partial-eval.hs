...
runPE src = do
  ...
  forM_ (sortBy (comparing (Down . nameOcc . varName . fst)) idsTerms) $ \(i,_) -> do
    print (nameOcc $ varName i, varUniq i)

  forM_ (sortBy (comparing (Down . nameOcc . varName . fst)) idsTerms) $ \(i,t) -> do
    putStrLn $ "Evaluating " <> show (nameOcc (varName i))
    print t
#if EXPERIMENTAL_EVALUATOR
    let ri   = mkRecInfo bm
    let env  = mkGlobalEnv i bm ri 10 (mempty, 0) tcm emptyInScopeSet ids
    print $ fst (runEval env (evaluateNf ghcEvaluator t))
#else
    let get (_, _, x) = x
